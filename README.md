...
The `scripts/` directory should contain `script1.sh`, `script2.sh` and `beagle.jar`
```bash
ls ${base_dir}/scripts
```
...
- Section A performs windowed local PCA across the chromosome, computes MDS on pairwise window distances, and identifies candidate inversion regions as MAD Z-score outliers on each MDS axis using a permutation test (adapted from Huang et al. 2010 Mol Ecol).
